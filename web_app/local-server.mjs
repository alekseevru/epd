...
async function loadDocumentStatistics(){
  const cacheKey="etrn-title2-title3-from-2026-09-v2";const cached=statisticsCache.get(cacheKey);if(cached?.expiresAt>Date.now())return cached.value;
  ...
  const items=await mapLimit(unique,8,async document=>{const flow=flowById.get(`${document.MessageId}:${document.EntityId}`);if(flow?.DocumentInfo?.IsDeleted)return null;const customData=Object.fromEntries((document.CustomData||[]).map(item=>[item.Key,item.Value]));let xml="";try{xml=await diadocEntityText(accessToken,config.boxId,document.MessageId,document.EntityId);}catch{}const deliveryDate=epdDateTime(xmlAttribute(xml,"ДатВрДостГр")||xmlAttribute(xml,"DeliveryDateTime"));if(Number.isNaN(deliveryDate.getTime())||deliveryDate>Date.now())return null;const titles=flow?.Docflow?.Titles||[],title1=titles.find(title=>Number(title.TitleIndex)===0),title2=titles.find(title=>Number(title.TitleIndex)===1),title3=titles.find(title=>Number(title.TitleIndex)===2),title1or2Signed=title1?.AuthorSigning?.IsFinished===true||title2?.AuthorSigning?.IsFinished===true,title2Signed=title2?.AuthorSigning?.IsFinished===true,title3Signed=title3?.AuthorSigning?.IsFinished===true;let title2Xml="";const title2EntityId=title2?.Version?.EntityId;if(title2EntityId&&title2EntityId!==document.EntityId){try{title2Xml=await diadocEntityText(accessToken,config.boxId,document.MessageId,title2EntityId);}catch{}}const firstParty=(source,...tags)=>tags.map(tag=>xmlParty(source,tag)).find(value=>value!=="Не указано")||"";const carrier=firstParty(title2Xml,"СвПер","СвИсп")||boxesById.get(String(title2?.Author?.BoxId||"").toLowerCase())||firstParty(xml,"СвПер","СвИсп")||"Не указано",consignee=boxesById.get(String(title3?.Author?.BoxId||"").toLowerCase())||firstParty(xml,"СвГП")||"Не указано";const rawDate=document.DocumentDate||document.CreationTimestamp||"";const match=String(rawDate).match(/^(\d{2})\.(\d{2})\.(\d{4})/);const parsed=match?new Date(`${match[3]}-${match[2]}-${match[1]}T00:00:00+03:00`):new Date(rawDate);const month=Number.isNaN(parsed.getTime())?"Не определён":`${parsed.getFullYear()}-${String(parsed.getMonth()+1).padStart(2,"0")}`;const containers=[...new Set(String(xml).match(/[A-Z]{4}\d{7}/g)||[])];return {messageId:document.MessageId,month,carrier,consignee,title1or2Signed,carrierTitle2Signed:title2Signed,consigneeTitle3Signed:title3Signed,number:document.DocumentNumber||xmlAttribute(xml,"НомерТрН")||"Без номера",containers,documentUrl:konturDocumentUrl(config.boxId,document.MessageId,customData["kl-id"]||document.EntityId,"LogisticsWaybill"),deliveryDate:new Intl.DateTimeFormat("ru-RU",{timeZone:"Europe/Moscow"}).format(deliveryDate)};});
  const merged=new Map();for(const item of items.filter(Boolean)){const hasStableNumber=item.number&&!/^(?:б\/н|Без номера)$/iu.test(item.number),logicalKey=hasStableNumber?`${item.month}:${item.number}`:item.messageId;const current=merged.get(logicalKey);if(!current){merged.set(logicalKey,item);continue;}current.title1or2Signed=current.title1or2Signed||item.title1or2Signed;current.carrierTitle2Signed=current.carrierTitle2Signed||item.carrierTitle2Signed;current.consigneeTitle3Signed=current.consigneeTitle3Signed||item.consigneeTitle3Signed;if(current.carrier==="Не указано"&&item.carrier!=="Не указано")current.carrier=item.carrier;if(current.consignee==="Не указано"&&item.consignee!=="Не указано")current.consignee=item.consignee;current.containers=[...new Set([...current.containers,...item.containers])];}
  const value={generatedAt:new Date().toISOString(),fromMonth:"2026-09",items:[...merged.values()]};statisticsCache.set(cacheKey,{expiresAt:Date.now()+10*60_000,value});return value;
}
