...
async function loadDocumentStatistics(){
  const cacheKey="etrn-carrier-title2-from-2026-09-v5";const cached=statisticsCache.get(cacheKey);if(cached?.expiresAt>Date.now())return cached.value;
  ...
  const items=await mapLimit(unique,8,async document=>{const flow=flowById.get(`${document.MessageId}:${document.EntityId}`);if(flow?.DocumentInfo?.IsDeleted)return null;let xml="";try{xml=await diadocEntityText(accessToken,config.boxId,document.MessageId,document.EntityId);}catch{}const deliveryDate=epdDateTime(xmlAttribute(xml,"ДатВрДостГр")||xmlAttribute(xml,"DeliveryDateTime"));if(Number.isNaN(deliveryDate.getTime())||deliveryDate>Date.now())return null;const titles=flow?.Docflow?.Titles||[],title2=titles.find(title=>Number(title.TitleIndex)===1),title2Signed=title2?.AuthorSigning?.IsFinished===true;let title2Xml="";const title2EntityId=title2?.Version?.EntityId;if(title2EntityId&&title2EntityId!==document.EntityId){try{title2Xml=await diadocEntityText(accessToken,config.boxId,document.MessageId,title2EntityId);}catch{}}const firstParty=(source,...tags)=>tags.map(tag=>xmlParty(source,tag)).find(value=>value!=="Не указано")||"";const carrier=firstParty(title2Xml,"СвПер","СвИсп")||boxesById.get(String(title2?.Author?.BoxId||"").toLowerCase())||firstParty(xml,"СвПер","СвИсп")||"Не указано";const rawDate=document.DocumentDate||document.CreationTimestamp||"";const match=String(rawDate).match(/^(\d{2})\.(\d{2})\.(\d{4})/);const parsed=match?new Date(`${match[3]}-${match[2]}-${match[1]}T00:00:00+03:00`):new Date(rawDate);const month=Number.isNaN(parsed.getTime())?"Не определён":`${parsed.getFullYear()}-${String(parsed.getMonth()+1).padStart(2,"0")}`;const containers=[...new Set(String(xml).match(/[A-Z]{4}\d{7}/g)||[])];return {messageId:document.MessageId,month,carrier,carrierTitle2Signed:title2Signed,number:document.DocumentNumber||xmlAttribute(xml,"НомерТрН")||"Без номера",containers,deliveryDate:new Intl.DateTimeFormat("ru-RU",{timeZone:"Europe/Moscow"}).format(deliveryDate)};});
  const merged=new Map();for(const item of items.filter(Boolean)){const hasStableNumber=item.number&&!/^(?:б\/н|Без номера)$/iu.test(item.number),logicalKey=hasStableNumber?`${item.month}:${item.number}`:item.messageId;const current=merged.get(logicalKey);if(!current){merged.set(logicalKey,item);continue;}current.carrierTitle2Signed=current.carrierTitle2Signed||item.carrierTitle2Signed;if(current.carrier==="Не указано"&&item.carrier!=="Не указано")current.carrier=item.carrier;current.containers=[...new Set([...current.containers,...item.containers])];}
