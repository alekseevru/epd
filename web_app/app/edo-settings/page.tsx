...
const confirmedParties=(parties:Party[])=>parties.flatMap(party=>{
  ...
  return option && (party.selectionSource==="manual" || (option.history?.count||0)>0) ? [{party,option,history:option.history}] : [];
});
...
export default function EdoSettings(){
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.content}><header className={styles.heading}><div><small>СПРАВОЧНИК</small><h1>Настройки ID для ЭПД</h1><p>В таблице — все контрагенты с несколькими ID ЭДО. Выбранный ID можно вручную подтвердить или изменить. В Excel попадают ID с подписанным документом или сохранённым ручным выбором.</p></div><div className={styles.headingActions}><button type="button" onClick={()=>void load()} disabled={loading}>{loading?"Проверяем…":"Обновить историю"}</button><button type="button" onClick={()=>void exportExcel()} disabled={loading||exporting||!confirmed.length}>{exporting?"Готовим Excel…":`Выгрузить подтверждённые (${confirmed.length})`}</button></div></header>
