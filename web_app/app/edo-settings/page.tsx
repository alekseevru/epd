...
export default function EdoSettings(){
  ...
  const load=async()=>{setLoading(true);try{const response=await fetch("/api/edo-directory",{cache:"no-store"});const result=await response.json();if(!response.ok)throw new Error(result.error);setParties(result.parties||[]);setMessage(`Контрагентов с несколькими операторами: ${(result.parties||[]).filter((party:Party)=>party.options.length>1).length}`);}catch(error){setMessage(error instanceof Error?error.message:"Не удалось загрузить справочник");}finally{setLoading(false);}};
  useEffect(()=>{void load();},[]);
  const filtered=useMemo(()=>{const value=query.trim().toLocaleLowerCase("ru-RU");const visible=parties.filter(party=>party.options.length>1);return !value?visible:visible.filter(party=>[party.name,party.inn,party.kpp,...party.options.flatMap(option=>[option.operator,option.id])].some(item=>String(item||"").toLocaleLowerCase("ru-RU").includes(value)));},[parties,query]);
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.content}><header className={styles.heading}><div><small>СПРАВОЧНИК</small><h1>Настройки ID для ЭПД</h1><p>В таблице — все контрагенты с несколькими ID ЭДО. Выбранный ID можно вручную подтвердить или изменить. В Excel попадают все контрагенты с одним ID ЭДО, а при нескольких ID — с подписанным документом или сохранённым ручным выбором.</p></div><div className={styles.headingActions}><button type="button" onClick={()=>void load()} disabled={loading}>{loading?"Проверяем…":"Обновить историю"}</button><button type="button" onClick={()=>void exportExcel()} disabled={loading||exporting||!confirmed.length}>{exporting?"Готовим Excel…":`Выгрузить настроенных (${confirmed.length})`}</button></div></header>
